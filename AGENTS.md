...
- Use `mise` for build orchestration and environment management (see `mise.toml`)
...
- The `mise.toml` tasks use `scripts/detect-os.sh` for proper OS detection (windows, linux, darwin)
...
- The mise tasks automatically select the correct scripts and binary extensions based on detected OS
...
- Use scripts in `mise.toml` tasks for complex or multi-step operations
...
- Reference them in `mise.toml` tasks with platform detection
